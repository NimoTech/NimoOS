...
readonly CASA_EXEC=nimoos
readonly CASA_CONF=/etc/nimoos/nimoos.conf
readonly CASA_URL=/var/run/nimoos/nimoos.url
readonly CASA_SERVICE_USR=/usr/lib/systemd/system/nimoos.service
readonly CASA_SERVICE_LIB=/lib/systemd/system/nimoos.service
readonly CASA_SERVICE_ETC=/etc/systemd/system/nimoos.service

# Old Casa Files
readonly CASA_PATH=/nimoOS
readonly CASA_CONF_PATH_OLD=/etc/nimoos.conf
...
Uninstall_Nimoos() {
    ...
    # Old Casa Files
    if [[ -d ${CASA_PATH} ]]; then
        rm -rvf ${CASA_PATH} || Show 1 "Failed to delete legacy NimoOS files."
    fi
    ...
    rm -rvf /var/lib/nimoos/app_category.json
    rm -rvf /var/lib/nimoos/app_list.json
    rm -rvf /var/lib/nimoos/docker_root
}
